...
(as the base reference) to calculate the distance of each spatial entity
from this reference base. It is common to use the minimum value as the
base reference (see references).
...
- X:
  ...
  A data frame or matrix with `n` rows (entities) and `m` columns
  (variables).

- reference_vector_function:

  Function used to build the reference vector. Common choices are `min`,
  `max`, `mean`, `median`, etc. See
  [`apply()`](https://rdrr.io/r/base/apply.html) for details. `min` is
  the default.

## Value

A vector with the reference value (base reference) for each variable.
Its length equals the number of variables.
